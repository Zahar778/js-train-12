...
function createDictionary(entries) {
  ...
  dictionary.forEach((value,key,map) => console.log(dictionary))
...
function transformDictionary(dictionary) {
  ...
  for (const obj of dictionary.entries()) {
    entries.push(obj);
  } 
...
function sortByValues(dictionary) {
  ...
  entriesArray.sort((big, small) => small[1] - big[1]);
